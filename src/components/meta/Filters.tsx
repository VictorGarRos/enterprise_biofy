...
export function MetaFilters({ onUpdate, loading = false }: FiltersProps) {
    ...
    return (
        <div className="premium-card p-6 flex flex-col xl:flex-row flex-wrap items-end gap-6 border border-zinc-700/30 shadow-2xl relative bg-zinc-900/40">
            <div className="absolute inset-0 backdrop-blur-xl rounded-[inherit] pointer-events-none -z-10" />
            <div className="flex flex-col gap-2 w-full md:w-64 relative" ref={accDropdownRef}
                ...
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest flex items-center gap-2 px-1">
                    <Users className="w-3 h-3 text-blue-500" /> Cuenta Publicitaria
                </label>
                <SelectButton 
            ...
            <div className="flex gap-2 w-full md:w-auto mt-2 xl:mt-0">
                <button
                    ...
                    disabled={loading}
                    className="flex-1 md:w-auto bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white px-8 py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 transition-all active:scale-95 shadow-xl shadow-blue-500/20"
                >
                ...
                <button
                    ...
                    disabled={loading}
                    className="flex-1 md:w-auto bg-zinc-800 border border-zinc-700 hover:bg-zinc-700 text-white px-6 py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 transition-all active:scale-95 shadow-sm"
...
function SelectButton({ label, isOpen, onClick }: any) {
    return (
        <button
            onClick={onClick}
            className="w-full flex items-center justify-between bg-zinc-800/50 hover:bg-zinc-800 border border-zinc-700/50 text-white p-3.5 rounded-xl transition-all duration-300 outline-none"
        >
...
function DropdownMenu({ options, selectedId, onSelect, searchable, searchTerm, setSearchTerm }: any) {
    ...
    return (
        <motion.div 
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-full left-0 w-full mt-2 bg-zinc-950 border border-zinc-700 shadow-2xl rounded-2xl z-[9999] overflow-hidden"
        >
            {searchable && (
                <div className="p-3 border-b border-zinc-800">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full bg-zinc-800 border-none text-white p-2.5 pl-9 rounded-lg outline-none text-xs placeholder:text-zinc-600 font-bold"
                        />
                    </div>
                </div>
            )}
            <div className="max-h-64 overflow-y-auto custom-scrollbar p-1.5 space-y-0.5">
                {filtered.map((o: any) => (
                    <button
                        ...
                        className={`w-full flex items-center justify-between p-3 rounded-lg text-left transition-colors ${selectedId === o.id ? 'bg-blue-600/10 text-blue-500 font-black' : 'text-zinc-400 font-bold hover:bg-zinc-800 hover:text-white'} text-xs`}
                    >
